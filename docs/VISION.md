...
// Streaming / session-oriented execution
...
|----------|-----------------------------------------------|---------|
| **OpenAI Codex** | `codex app-server` — JSON-RPC über stdio/JSONL (optional WebSocket), Handshake via `initialize`, dann Threads (`thread/start`, `thread/resume`, `thread/fork`) und Turns (`turn/start`, `turn/steer`), **serverseitige Approvals**, gestreamte `item/*`- und `turn/*`-Events | Offiziell die Deep-Integration-Klasse ([Codex App Server](https://developers.openai.com/codex/app-server)); Implementierung offen im [Codex-Repo](https://github.com/openai/codex). |
...
### Die Landschaft (Stand Frühjahr 2026)
...
**Phasen A-K (abgeschlossen):** Schema, Orchestrator, Dashboard-Grundlage, Runner, Extension, Multi-Turn-Basis und Projekt-Import.

**Phase L (substanziell umgesetzt):** Connector-Architektur, `AgentEvent`-Modell, Connectors für **Claude Code**, **Codex**, **Gemini**, **HTTP** und **Cursor/ACP**, Orchestrator-Approve/Deny-Flow, Session-Streaming, Dashboard-API-Routen, Agent Chat, Wintermute-Relay, persistente Sessions unter `.project-agents/sessions/`, `/sessions` als primäre Arbeitsfläche und connector-first Dispatch im Produkt.

**Produktzustand heute:** Der Standardfluss ist `Task -> Session -> Review`. `codex app-server` ist der bevorzugte Default-Connector. Provider-Session-IDs, Resume/Fork-Semantik und die eingebettete Dashboard-Nutzung in Wintermute sind Teil des aktuellen Produktpfads, nicht mehr nur Roadmap.

Details: `./PLAN.md` Phase L, Provider-Tabelle oben.

### Aktueller Fokus

- **Sessions als Workspace weiter schärfen** — Transcript, Tool-Use, Permissions und Meta-Rail weiter als echte Agent-Workspace-UI ausbauen.
- **Runner als sekundären Pfad robust halten** — `One-off Run` bleibt wichtig für Fallback, Automation und Diagnose.
- **Build-/Test-Härtung** — CI-Stabilität und lokale Testpfade weiter robuster machen.
- **Doku-Disziplin** — Vision, README, Plan und Implementierung bei Änderungen gemeinsam fortschreiben.
...
- **Nutzerdefinierte MCP-Server** — projekt- oder userseitig zusätzliche MCP-Server registrieren, damit Agents gezielt auf freigegebene Tools und Datenquellen zugreifen können
