...
export function Sidebar() {
    ...
    return (
        <aside className="fixed inset-y-0 left-0 w-64 glass border-r border-surface-900 flex flex-col z-50">
            <div className="p-6 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-brand-400">Control Plane</p>
                </div>
            </div>
            ...
            <nav className="flex-1 px-4 py-4 space-y-1">
                {navItems.map((item) => (
                    <Link
                        key={item.label}
                        href={item.href}
                        className="flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium text-surface-400 hover:text-white hover:bg-surface-900/50 transition-all duration-200 group relative overflow-hidden"
                    >
                        <div className="absolute inset-y-0 left-0 w-1 bg-brand-500 transform -translate-x-full group-hover:translate-x-0 group-hover:shadow-[0_0_10px_rgba(92,129,163,0.5)] transition-transform duration-300" />
