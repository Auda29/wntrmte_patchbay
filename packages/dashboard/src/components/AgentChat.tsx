...
import type { Run } from '@patchbay/core';

const fetcher = (url: string) => fetch(url).then((res) => res.json());

type AgentEventMessage =
  | { type: 'session:started'; sessionId: string; connectorId: string; timestamp: string }
  | { type: 'agent:message'; content: string; partial?: boolean; timestamp: string }
  | { type: 'agent:tool_use'; toolName: string; status: 'started' | 'completed' | 'failed'; toolOutput?: string; timestamp: string }
  | { type: 'agent:permission'; description: string; permissionId: string; timestamp: string }
  | { type: 'agent:question'; question: string; timestamp: string }
  | { type: 'session:completed'; summary?: string; timestamp: string }
  | { type: 'session:failed'; error: string; timestamp: string }
  | { type: 'stream:end' };
...
export function AgentChat({ open, taskId, taskTitle, onClose }: AgentChatProps) {
  ...
  const { data, mutate } = useSWR<Run[]>(
    open ? `/api/runs?taskId=${encodeURIComponent(taskId)}` : null,
    fetcher,
    { refreshInterval: open ? 2000 : 0 }
  );
  ...
  const hasLiveSession = activeRun?.status === 'running' || activeRun?.status === 'awaiting_input';
  ...
  useEffect(() => {
    ...
    source.onmessage = (message) => {
      try {
        const parsed = JSON.parse(message.data) as AgentEventMessage;
        setEvents((current) => {
          if (parsed.type === 'agent:message' && parsed.partial && current.at(-1)?.type === 'agent:message') {
        ...
        if (parsed.type === 'session:completed' || parsed.type === 'session:failed' || parsed.type === 'stream:end') {
          void mutate();
        }
      } catch {
  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-40 flex w-full max-w-2xl flex-col border-l border-surface-800/80 bg-[linear-gradient(180deg,rgba(10,14,20,0.98)_0%,rgba(13,17,23,0.96)_100%)] shadow-[-20px_0_48px_rgba(0,0,0,0.28)] backdrop-blur-xl">
      <div className="flex items-start justify-between gap-4 border-b border-surface-800/70 px-6 py-5">
        <div>
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-brand-300">Agent Chat</p>
          <h2 className="mt-1 text-xl font-semibold text-surface-50">{taskTitle}</h2>
          <p className="mt-1 text-sm text-surface-400">{taskId}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="border-b border-surface-800/60 px-6 py-3 text-sm text-surface-400">
        {sessionId
          ? hasLiveSession
            ...
            : `Last session ${sessionId} is no longer active.`
          : 'No active interactive session yet. Start one from Dispatch.'}
      </div>
        ...
        {events.length === 0 ? (
          <div className="glass-card rounded-xl border border-surface-800/70 p-5 text-sm text-surface-400">
            {sessionId
              ...
              : 'Open Dispatch, choose a connector-capable provider, and start an interactive session.'}
          </div>
        ) : (
          events.map((event, index) => {
            if (event.type === 'session:started') {
              return (
                <div key={`${event.type}-${index}`} className="rounded-xl border border-surface-800/70 bg-surface-950/50 px-4 py-3 text-sm text-surface-300">
            ...
            if (event.type === 'agent:permission') {
              return (
                <div key={`${event.type}-${index}`} className="rounded-xl border border-yellow-900/50 bg-yellow-950/20 px-4 py-4">
            ...
            if (event.type === 'agent:question') {
              return (
                <div key={`${event.type}-${index}`} className="rounded-xl border border-brand-900/50 bg-brand-950/20 px-4 py-4 text-sm text-brand-100">
                  {event.question}
                </div>
              );
            }
            ...
            if (event.type === 'session:completed') {
              return (
                <div key={`${event.type}-${index}`} className="rounded-xl border border-green-900/50 bg-green-950/20 px-4 py-4 text-sm text-green-100">
            ...
            if (event.type === 'session:failed') {
              return (
                <div key={`${event.type}-${index}`} className="rounded-xl border border-red-900/50 bg-red-950/20 px-4 py-4 text-sm text-red-100">
                  {event.error}
                </div>
              );
            }
